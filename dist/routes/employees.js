...
router.get("/employees", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), async (req, res) => {
    ...
    try {
        ...
        const formattedEmployees = employees.map((emp) => ({
            ...
            lastCheckIn: emp.lastCheckIn?.toISOString(),
            lastCheckOut: emp.lastCheckOut?.toISOString(),
...
// Check-in endpoint (employees only)
router.post("/check-in", auth_js_1.auth, (0, role_js_1.requireRole)("OPERATOR"), async (req, res) => {
    try {
...
// Check-out endpoint (employees only)
router.post("/check-out", auth_js_1.auth, (0, role_js_1.requireRole)("OPERATOR"), async (req, res) => {
    try {
        const userId = req.user.id;
        const employee = await db_1.default.employee.findUnique({
            where: { userId },
        ...
        res.json({
            ...
            isCheckedIn: updated.isCheckedIn,
            lastCheckOut: updated.lastCheckOut,
        });
    }
    catch (err) {
...
// Get check-in status for current employee
router.get("/my-status", auth_js_1.auth, (0, role_js_1.requireRole)("OPERATOR"), async (req, res) => {
    try {
        const userId = req.user.id;
        const employee = await db_1.default.employee.findUnique({
            where: { userId },
...
// Get all employees status (managers only, for polling)
router.get("/status", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), async (req, res) => {
    try {
        const managerId = req.user.id;
        const employees = await db_1.default.employee.findMany({
            where: { managerId },
