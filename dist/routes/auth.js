...
router.post("/dev-login", async (req, res) => {
    try {
        ...
        // Validate role
        const validRoles = ["MANAGER", "OPERATOR"];
        if (!validRoles.includes(devUser.role)) {
            return res.status(403).json({
                error: "Invalid role",
                message: "User role is not authorized for this application"
            });
        }
        ...
        const appToken = jwt.sign({
            ...
            email: user.email,
            type: "dev-login",
            permissions: devUser.role === "MANAGER"
                ? ["read", "write", "manage_employees", "assign_tasks"]
                : ["read", "update_own_tasks"]
        }, process.env.JWT_SECRET || "dev-secret-key-change-in-production", { expiresIn: "7d" });
